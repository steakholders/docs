...
La richiesta viene gestita da \emph{Authentication} che verifica con esito negativo i permessi e rimanda un next(error) al router.
\begin{figure}[H]
	\begin{center} 
...
Nel diagramma seguente viene rappresentato lo scenario in cui si richiedono permessi ''Admin'' per poter gestire la richiesta corrispondente e la verifica effettuata dal middleware \emph{Authentication} fallisce. 
\begin{figure}[H]
	\begin{center} 
...
Nel diagramma seguente viene rappresentato lo scenario in cui si richiedono permessi ''Super Admin'' per poter gestire la richiesta corrispondente e la verifica effettuata dal middleware \emph{Authentication} fallisce. 
\begin{figure}[H]
	\begin{center} 
...
Il seguente scenario mostra la gestione di una richiesta POST per la risorsa profile, richiamato \emph{requireNotLogged()}, \emph{Authentication} non risponde con errore, chiamando successivamente \emph{ProfileService} che gestisce la verifica dei parametri e nell'opzione che questa fallisca, manda in risposta un next(error) che il router andrà a gestire.
\begin{figure}[H]
	\begin{center} 
...
La verifica dei permessi in \emph{Authentication} non fallisce, innescando la chiamata al successivo \glossario{middleware} \emph{ProfileService} che gestisce l'eliminazione della risorsa.
\begin{figure}[H]
	\begin{center} 
...
Il seguente diagramma rappresenta lo scenario di una richiesta GET per ottenere la risorsa Profile, la verifica dei permessi non fallisce e la richiesta viene gestita da \emph{ProfileService}.
\begin{figure}[H]
	\begin{center} 
...
Viene rappresentato lo scenario di una richiesta PUT per la risorsa Profile, il \emph{requireLogged()} non fa ritornare un errore, viene chiamato il successivo \glossario{middleware} \emph{ProfileService} chiamando il metodo \emph{updatePassword()} che gestisce la richiesta di modifica della password del profilo.
Nell'opzione che i parametri passati per la modifica del profilo siano errati, \emph{updatePassword()} chiamerà la callback passandogli la descrizione dell'errore come parametro che il router andrà a gestire.
\begin{figure}[H]
	\begin{center} 
...
Viene rappresentato lo scenario di una richiesta POST per la risorsa password forgot, \emph{Authentication} non risponde errore e il controllo passa a \emph{ForgotService} che gestisce la richiesta.
Se i parametri passati per la richiesta sono errati, il service \emph{Authentication} risponderà con un errore.
\begin{figure}[H]
	\begin{center} 
...
Viene rappresentato nel seguente diagramma di sequenza lo scenario di una richiesta GET per la risorsa user, \emph{requireAdmin()} non fallisce ed il controllo viene passato a \emph{UserService} che gestisce la richiesta.
\begin{figure}[H]
	\begin{center} 
...
Il seguente diagramma di sequenza rappresenta lo scenario di una richiesta POST per la risorsa user, la verifica di \emph{requireLogged()} dei permessi utente non fallisce e viene passato il controllo a \emph{UserService} tramite chiamata del metodo \emph{createUser()} che gestisce la richiesta di creazione di un nuovo user, e nel caso la verifica dei parametri passati per la creazione di quest'ultimo siano errati, il service chiamerà la callback con l'errore.
\begin{figure}[H]
	\begin{center} 
...
Lo scenario rappresenta una richiesta GET di una risorsa User id, vengono verificati i permessi attraverso \emph{requireAdmin()} che passa poi il controllo a \emph{userService} invocando il metodo \emph{getUser()} dandogli come attributo l'id dell'user da restituire.
...
Nel seguente diagramma di sequenza viene rappresentato lo scenario di una richiesta PUT per la risorsa User id nel quale \emph{requireAdmin()} non restituisce un errore e passa il controllo a \emph{userService} che gestisce la richiesta di modifica del livello corrispondente all'userId che gli è stato passato come attributo.
\emph{updateLevel()} verifica inoltre che l'userId passatogli come attributo non corrisponda all'id dello stesso user che ha effettuato la richiesta o corrisponda ad un id di un superAdmin e controlla che i parametri passati non siano errati, altrimenti risponderà con errore.
\begin{figure}[H]
	\begin{center} 
...
Nell'opzione che l'id passatogli corrisponda all'id dell'utente che ha effettuato la richiesta o ad un id di un superAdmin, \emph{UserService} restituisce un next(error). Il service si preoccupa inoltre di verificare che i parametri siano corretti.
\begin{figure}[H]
	\begin{center} 
...
Il diagramma seguente rappresenta lo scenario di una richiesta GET per la risorsa collection, il service \emph{Authentication} innescherà la chiamata del successivo service \emph{CollectionService} che gestirà la richiesta di restituzione della lista di \glossario{Collection}.
\begin{figure}[H]
	\begin{center} 
...
Il diagramma seguente rappresenta lo scenario di una richiesta GET per la risorsa collection Name, il service \emph{Authentication} innescherà la chiamata del successivo service \emph{indexService} al quale verrà passato come parametro l'id della \glossario{Collection} per la restituzione dell'index page corrispondente. \\ 
...
Il diagramma di sequenza rappresenta lo scenario di una richiesta GET per la risorsa collection name document, nel quale al service \emph{showService} viene passato l'id del document di cui mostrare la show page.
Nell'opzione l'id sia errato, non corrispondendo ad un \glossario{Document} valido, \emph{showService} restituisce una next(error).
\begin{figure}[H]
	\begin{center} 
...
Il seguente scenario rappresenta la richiesta DELETE per una risorsa Collection name document, dopo che i permessi sono stati verificati il controllo è passato a \emph{ShowService} il quale gestirà la richiesta di eliminazione del document il cui id gli è stato passato come parametro. Se l'id è errato, verrà restituito un errore.
\begin{figure}[H]
	\begin{center} 
...
\begin{itemize}
	 \item \textbf{Passport}: è un middleware il cui unico scopo è gestire le richieste di autenticazione per \glossario{Node.js}. Estremamente flessibile e modulare, Passport può essere facilmente inserito in qualsiasi applicazione web basata su Express. Con Passport è possibile cambiare il meccanismo di autenticazione, ovvero la \emph{strategia} da adottare senza creare dipendenze superflue, scegliendo tra quelle proposte dalla libreria o personalizzandone per adattarle alle proprie esigenze. Inoltre, Passport mantiene persistentemente le sessioni di login e permette di configurare agevolmente le azioni in caso di fallimento o successo dell'autenticazione;
	 \item \textbf{Passport-local}: è una libreria che permette di autenticare un utente con Passport identificandosi come una \emph{strategia} precedentemente citata, utilizzando uno username e una password. Come Passport, si integra facilmente con qualsiasi applicazione o framework che supporta lo stile di connessione tra le componenti tramite il middleware, incluso Express;
	 \item \textbf{Passport-local-mongoose}: è un plugin per Mongoose che semplifica la costruzione di un sistema di autenticazione con Passport. Permette di inserire l'email per l'autenticazione, aggiunge allo schema l'hash della password e il valore salt;
	 \item \textbf{Nodemailer}: è un modulo che permette di mandare facilmente e-mail con Node.js tramite \glossario{SMTP}. Tra le varie caratteristiche, supporta i caratteri \glossario{Unicode}, contenuto \glossario{HTML} e testo non formattato, allegati, immagini integrate nel codice \glossario{HTML}, i protocolli di comunicazione \glossario{SSL} e \glossario{STARTTLS};
	 \item \textbf{Sweet.js}: è un modulo che permette di definire macro utilizzando la sintassi del linguaggio Javascript. Con Sweet.js è possibile modellare un linguaggio \textit{ad hoc} basato su JavaScript, collezionare le macro in un singolo modulo e condividerlo sul \glossario{Node Packaged Modules}.
\end{itemize}