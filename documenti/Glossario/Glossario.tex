...
\definizione{Application logic}
Parte del sistema che si occupa di processare i dati, e che non interagisce con l'\glossario{utente}. Solitamente risiede nel \glossario{back-end}, ma può anche risiedere nel \glossario{front-end}.
...
\definizione{Factory}
\glossario{Design pattern} creazionale che fornisce un interfaccia per creare una famiglia di oggetti senza specificare classi concrete.
...
Vedi \glossario{linguaggi funzionali}.
...
\definizione{Parser}
Strumento che si occupa di fare il \glossario{parsing}.
...
\definizione{SSL}
Protocollo crittografico che permette una comunicazione sicura dal sorgente al destinatario (end-to-end) su reti TCP/IP fornendo autenticazione, integrità dei dati e cifratura operando al di sopra del livello di trasporto.
...
\definizione{STARTTLS}
Estensione dei protocolli di comunicazione che permette di crittografare la connessione (ad esempio con \glossario{SSL}) senza utilizzare una porta diversa.
...
\definizione{Design pattern} comportamentale il cui compito è isolare un algoritmo all'interno di un oggetto.
...
\definizione{Viste}
Interfaccia utente nel modello \glossario{MVC}.
