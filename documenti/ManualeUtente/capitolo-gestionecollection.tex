\section{Gestione delle collection}
	...
	La \glossario{collection} index è la pagina in cui viene visualizzato il contenuto di una \glossario{collection} in forma tabellare. Per aprire una \glossario{collection} index (vedi figura \ref{fig:collection}) è necessario cliccare sul relativo link presente nella barra dei menù (1), MaaP visualizzerà l'intero contenuto della \glossario{collection} (2). La tabella visualizza i documenti dei quali è possibile aprire la \glossario{showpage} (vedi \ref{aperturashowpage}) cliccando sui relativi link (3). %TODO \`E possibile filtrare i contenuti della pagina tramite il pannello situato sulla destra.
	...
	La \glossario{showpage} di un \glossario{document} visualizza gli attributi del documento in forma tabellare. Per accedere ad una show page è necessario posizionarsi nella \glossario{collection} index di appartenenza (vedi \ref{aperturacollectionindex}) e seguire il punto 3.
