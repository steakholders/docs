...
Viene rappresentato lo scenario di una richiesta POST per la risorsa password lost, il \emph{requireNotLogged} non risponde errore e il controllo passa a \emph{requirePasswordReset} che gestisce la richiesta.
\begin{figure}[H]
	\begin{center} 
...
Il diagramma di sequenza rappresenta lo scenario di una richiesta PUT per la risorsa password reset, dove il controller \emph{requireNotLogger} passa il controllo al successivo controller \emph{makePasswordReset} per la gestione della richiesta.
\begin{figure}[H]
	\begin{center} 
