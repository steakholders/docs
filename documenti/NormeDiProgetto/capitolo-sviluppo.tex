\section{Processo di sviluppo}
    ...
    Lo scopo del processo di sviluppo è la produzione di un elemento di un sistema implementato come prodotto software.
    Questo processo trasforma specifici comportamenti, interfacce, e vincoli di implementazione in azioni che creano un sistema implementato come prodotto software.
    Questo processo produce un software che soddisfa i requisiti architetturali e li verifica attraverso la verifica e la validazione.

    \subsection{Risultati osservabili del processo}
    I risultati di una corretta implementazione di tale processo sono:
    \begin{itemize}
        \item una strategia di sviluppo definita;
        \item i vincoli tecnologici e di design sono identificati;
        \item il sistema software è stato realizzato.
    \end{itemize}

    \subsection{Descrizione}
    Il processo di sviluppo, in accordo con lo standard di aderenza, si compone dei seguenti sotto processi:
    \begin{itemize}
        \item Processo di analisi dei requisiti (vedi \ref{processoAR});
        \item Processo di progettazione (vedi \ref{processoProgettazione});
        \item Processo di codifica (vedi \ref{processoCodifica}).
    \end{itemize}
...
\label{processoAR}
    ...
    Tale processo ha l'obiettivo di formalizzare e rendere tracciabile in un documento i requisiti e casi d'uso individuati.

    \subsubsection{Risultati osservabili del processo}
    Con il completamento di questo processo si ottiene una documentazione affidabile e consistente che ben descriva le esigenze e le richieste degli \glossario{stakeholders}.
...
\label{processoProgettazione}

    \subsubsection{Scopo del processo}
    Il processo di progettazione ha come obiettivo la stesura di:
    \begin{itemize}
        \item Specifica Tecnica: intende descrivere l'architettura del software e descrive lo stack tecnologico utilizzato;
        \item Definizione di Prodotto: descrive nel modo più dettagliato possibile la progettazione;
    \end{itemize}
    Negli scopi di questo processo rientra anche una vasta produzione di diagrammi \glossario{UML}.

    \subsubsection{Risultati osservabili del processo}
    Questo processo porta alla produzione di documentazione affidabile, completa e coerente.
...
\subsubsection{Risultati osservabili del processo}
Questo processo porta a un software stabile, affidabile, funzionale e aderente con quanto richiesto dagli \glossario{stakeholders}.

\subsubsection{Descrizione}
La codifica avviene secondo i task assegnati specificati nel \PianoDiProgetto{} e utilizzando gli strumenti indicati in \ref{strumentiambientedisviluppo}.
