...
\begin{itemize}
 \item \textbf{gestione delle attività}: viene fatta utilizzando la sezione ``Task'' di \glossario{TeamworkPM}
 \item \textbf{gestione delle modifiche}: viene fatta utilizzando la sezione ``Issues'' di \glossario{GitHub}
\end{itemize}
...
Il Responsabile deve valutare ogni nuova \glossario{issue}. Se ritiene che la formulazione sia corretta deve segnarle come approvate:
\begin{itemize}
 \item \textbf{Tag}: viene aggiunto \textbf{approvato}
\end{itemize}

Se è necessario approfondire la modalità in cui verrà eseguita l'attività richiesta allora il Responsabile deve anche assegnare la \glossario{issue} ad un ruolo competente:
\begin{itemize}
...
Il Responsabile deve pianificare le \glossario{issue} a lui assegnate e marcate ``approvato'' o ``progettazione'' creando un \glossario{task}. Deve inoltre pianificare il corrispondente \glossario{task} di verifica se necessario. I \glossario{task} del compito e della modifica devono essere assegnati a persone diverse.
...
Inoltre deve chiudere ogni eventuale \glossario{issue} associata:
\begin{itemize}
